...
// 탭바 페이지들 정의
const TAB_PAGES = ['/shop', '/assets', '/wallet', '/dashboard', '/benefits', '/menu'];

// 페이지 전환 방향을 결정하는 로직
const getTransitionDirection = (fromPath: string, toPath: string): 'forward' | 'backward' | 'none' => {
  // 탭바 간 전환은 애니메이션 없음 (더 자연스러운 UX)
  if (TAB_PAGES.includes(fromPath) && TAB_PAGES.includes(toPath)) {
    return 'none';
  }
  // 페이지 계층 구조 정의 (깊이가 클수록 forward) - KB 스타뱅킹 원본 구조
  const pageDepths: Record<string, number> = {
    ...
    // 서브 페이지들
    '/account': 2,                        // 계좌 목록
    '/account/:accountId': 3,             // 계좌 상세
    '/account/:accountId/transactions': 4, // 계좌별 거래내역
    '/comprehensive-account': 3,           // 통합계좌조회
    '/transfer': 2,                       // 이체
    '/transfer/picture': 3,               // 사진 이체
    '/transactions': 2,                   // 거래내역
  };
