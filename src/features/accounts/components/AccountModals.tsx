...
const TransactionName = styled(Typography.Title)`
  text-align: center;
...
`;
const MemoLabel = styled(Typography.Caption)`
...
const DetailDateTime = styled(Typography.Body)`
...
`;
const DetailLabel = styled(Typography.Body)`
  color: ${tokens.colors.text.secondary};
`;
const DetailValue = styled(Typography.Body)`
  font-weight: 500;
`;
const DetailAmount = styled(Typography.Body)<{ type: 'income' | 'expense' }>`
  font-weight: 600;
  color: ${props => props.type === 'income' ? '#4CAF50' : '#333'};
`;
